...
import { homeLoanEmiCalculationFormDataScheme } from "@/schemas";
import {
...
import {HomeLoanEmiCalculation} from "@prisma/client";

const defaultValues: IHomeLoanEmiCalculationFormData = {
    loanAmount: 1000000,
    rateOfInterest: 6.5,
    loanTenure: 5,
    tenureType: 12,
...
const Calculator = () => {
  const form = useForm<IHomeLoanEmiCalculationFormData>({
    resolver: zodResolver(homeLoanEmiCalculationFormDataScheme),
    defaultValues,
  });
  ...
  >({
    apiUrl: HOME_LOAN_EMI_CALCULATIONS_API_URL,
    queryKey: HOME_LOAN_EMI_CALCULATIONS_QUERY_KEY,
    defaultValues,
    form,
  ...
  const {
    ...
    isFetching,
  } = useQuery<HomeLoanEmiCalculation[] | null>({
    queryKey: [HOME_LOAN_EMI_CALCULATIONS_QUERY_KEY],
    queryFn: () => getCalculations(HOME_LOAN_EMI_CALCULATIONS_API_URL),
